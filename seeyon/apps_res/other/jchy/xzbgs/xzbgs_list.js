...
function doGwmod() {
    ...
    if (count == 1) {
        var obj = rows[0];
        var url= _ctxPath + '/demo.do?method=toXzbgsMod&id='+obj.formid+'&summaryid='+obj.summaryid;
...
function doGwmodView(formid,summaryid) {
    var url= _ctxPath + '/demo.do?method=toXzbgsMod&id='+formid+'&summaryid='+summaryid;
    var options = "status=no,resizable=no,menubar=no,top=10,left=200,width=1073,height=742,scrollbars=no,center:Yes;";
    window.open(url, null, options);
}
    ...
    var confirm = $.confirm({
        ...
        ok_fn: function () {
            ...
            $.ajax({
                ...
                type:'POST',
                data:{params:simIds,tablename:'formmain_0285',summarytable:'col_summary'},
