...
$(document).ready(function () {
    ...
    //编辑
    toolbarArray.push({id: "ceshi",name:"校内请示修改",className: "ico16 editor_16",click:doGwmod});
    ...
    //请示标题
    condition.push({id: 'qsbt',name: 'qsbt',type: 'input',text: '请示标题',value: 'qsbt',maxLength:100});
    //起草人
    condition.push({id: 'qcr',name: 'qcr',type: 'input',text: '起草人',value: 'qcr',maxLength:100});
...
function doGwmod() {
    ...
    if (count == 1) {
        var obj = rows[0];
        var url= _ctxPath + '/demo.do?method=toXnqsMod&formid='+obj.formid+'&summaryid='+obj.summaryid;
...
function doGwmodView(formid,summaryid){
    var url= _ctxPath + '/demo.do?method=toXnqsMod&formid='+formid+'&summaryid='+summaryid;
    var options = "status=no,resizable=no,menubar=no,top=10,left=200,width=1073,height=742,scrollbars=no,center:Yes;";
    window.open(url, null, options);
}
